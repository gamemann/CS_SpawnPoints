...
{
	// ConVars
	g_hTSpawns = CreateConVar("sm_SP_spawns_t", "32", "Amount of spawn points to enforce on the T team.");
	g_hCTSpawns = CreateConVar("sm_SP_spawns_ct", "32", "Amount of spawn points to enforce on the CT team.");
	g_hTeams = CreateConVar("sm_SP_teams", "1", "0 = Disabled, 1 = All Teams, 2 = Terrorist only, 3 = Counter-Terrorist only.");
	g_hCourse = CreateConVar("sm_SP_course", "1", "1 = When T or CT spawns are at 0, the opposite team will get double the spawn points.");
	g_hDebug = CreateConVar("sm_SP_debug", "0", "1 = Enable debugging.");
	g_hAuto = CreateConVar("sm_SP_auto", "0", "1 = Add the spawn points as soon as a ConVar is changed.");
	g_hMapStartDelay = CreateConVar("sm_SP_mapstart_delay", "1.0", "The delay of the timer on map start to add in spawn points.");
	
	// AlliedMods Release
	CreateConVar("sm_SP_version", PL_VERSION, "Spawn Points version.");
	...
	AutoExecConfig(true, "sm_SpawnPoints");
}
...
public Action:Command_AddSpawns(iClient, sArgs) {
	...
	if (iClient == 0) {
		...
		PrintToChat(iClient, "\x02[SP] \x03Added map spawns!");
	}
...
stock AddMapSpawns() {
	...
	if (g_bcvarDebug) {
		LogMessage("There are %d/%d CT points and %d/%d T points", iCTSpawns, g_icvarCTSpawns, iTSpawns, g_icvarTSpawns);
	}
